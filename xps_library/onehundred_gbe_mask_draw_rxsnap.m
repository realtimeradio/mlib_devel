...
function onehundred_gbe_mask_draw_rxsnap(cursys, pipe_no_pipe)
...
if strcmp(snaplen, '0 - no snap') == 0
    ...
    reuse_block(cursys, 'rxs4', 'casper_library_scopes/bitfield_snapshot', ...
        'Position', [1620        2436        1725        2664], ...
        'io_names', '[data_lsw]', ...
        'io_widths', '[128]', 'io_bps', '[0]', ...
        'io_types', '[0]', 'snap_storage', 'bram', ...
        'snap_dram_dimm', '2', 'snap_dram_clock', '250', ...
        ...
        'extra_names', '[notused]', 'extra_widths', '[32]', ...
        'extra_bps', '[0]', 'extra_types', '[0]');
    try set_param([cursys, '/rxs4'], 'snap_ext_arm', 'on'); catch, end
    try set_param([cursys, '/rxs4'], 'snap_ext_circ', 'off'); catch, end
    if rx_snap_case == 3
        reuse_block(cursys, 'rxs5', 'casper_library_scopes/bitfield_snapshot', ...
            'Position', [1620        2721        1725        2949], ...
            ...
            'extra_names', '[notused]', 'extra_widths', '[32]', ...
            'extra_bps', '[0]', 'extra_types', '[0]');
        try set_param([cursys, '/rxs5'], 'snap_ext_arm', 'on'); catch, end
        try set_param([cursys, '/rxs5'], 'snap_ext_circ', 'off'); catch, end
    else
        delete_block_lines_s([cursys, 'rxs5']);
        delete_block_s([cursys, '/rxs5']);
    end
    ypos = 1310;
    ...
    if rx_snap_case == 3
        reuse_block(cursys, 'rxs5_del0',  ...
            'xbsIndex_r4/Delay', 'reg_retiming', 'on', 'latency', '1', ...
            'Position', [1570        2190        1590        2210]);
        reuse_block(cursys, 'rxs5_del1',  ...
            'xbsIndex_r4/Delay', 'reg_retiming', 'on', 'latency', '1', ...
            'Position', [1570        2235        1590        2255]);
        add_line_s(cursys, 'rxs5_del0/1', 'rxs5/1');
        add_line_s(cursys, 'rxs5_del1/1', 'rxs5/2');
    else
        delete_block_lines_s([cursys, 'rxs5_del0']);
        delete_block_lines_s([cursys, 'rxs5_del1']);
        delete_block_s([cursys, '/rxs5_del0']);
        delete_block_s([cursys, '/rxs5_del1']);
    end
    ...
    add_line_s(cursys, 'rxs_exp/3',     'rxs3/1');
    add_line_s(cursys, we_del,          'rxs3/2');
    add_line_s(cursys, 'rxsnap_and/1',  'rxs3/3');
    add_line_s(cursys, 'rxs0/1',        'rxs3/4');

    add_line_s(cursys, 'rxs_exp/4',     'rxs4/1');
    add_line_s(cursys, we_del,          'rxs4/2');
    add_line_s(cursys, 'rxsnap_and/1',  'rxs4/3');
    add_line_s(cursys, 'rxs0/1',        'rxs4/4');
    
    if rx_snap_case == 3
        add_line_s(cursys, [clear_name([pipe_no_pipe, '_rx_source_ip']), '/1'], 'rxs5_del0/1');
        add_line_s(cursys, [clear_name([pipe_no_pipe, '_rx_dest_ip']), '/1'], 'rxs5_del1/1');
        add_line_s(cursys, we_del,          'rxs5/3');
        add_line_s(cursys, 'rxsnap_and/1',  'rxs5/4');
        add_line_s(cursys, 'rxs0/1',        'rxs5/5');
    end
else
    for ctr = 1 : 8
        delete_block_lines_s([cursys, '/rxs_', num2str(ctr), '_del']);
    ...
    delete_block_lines_s([cursys, '/rxs4']);
    delete_block_lines_s([cursys, '/rxs5']);
