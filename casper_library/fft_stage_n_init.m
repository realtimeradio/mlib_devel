...
if strcmp(getenv('USE_DUMB_BRAMS'), '1'),
    USE_DUMB_BRAMS = 1;
else
    USE_DUMB_BRAMS = 0;
end
...
biplex_mux_latency= get_var('biplex_mux_latency', 'defaults', defaults, varargin{:});
...
mux_latency = biplex_mux_latency;
...
if strcmp(delays_bram, 'on'),
  ...
  reuse_block(blk, 'delay0', 'casper_library_bus/bus_delay', ...
    'n_bits', num2str(input_bit_width * n_inputs), 'cmplx', 'on', 'misc', 'off', ...
...
reuse_block(blk, 'butterfly_direct', 'casper_library_ffts/butterfly_direct', ...
    ...
    'shift_mux_latency', num2str(shift_mux_latency), ...
