...
function coeff_gen_init(blk, varargin)
  ...
  if length(ActualCoeffs) == 1,
    ...
    if (inorder == 1) || strcmp(coeff_generation, 'off'),
      ...
      if strcmp(hide_latency, 'on'), count_offset=bram_latency;
      else, count_offset = 0;
      end

      reuse_block(blk, 'Counter', 'xbsIndex_r4/Counter', ...
          'cnt_type', 'Free Running', 'start_count', num2str(count_offset), 'cnt_by_val', '1', ...
      ...
      reuse_block(blk, 'cosin', 'casper_library_downconverter/cosin', ...
        ...
        'bram_latency', num2str(bram_latency), 'add_latency', '0', ...
        'mux_latency', '0', 'neg_latency', '0', 'conv_latency', '0', ...
        'store', num2str(store), 'pack', pack, 'bram', bram, 'misc', cosin_misc, ...
	'misc_extra_latency', num2str(-count_offset), ...
